...
/*Average Ticket */
PROC SQL;
create table adhoc5.merchant_list as
select MERCHANT_NAME,count(*) as count
from targpord.crd_trxn_ods_data
group by 1
union all 
select distinct merch_name,count(*) as count
from dcdm.ptlf_data
group by 1;
quit;

data adhoc5.EOSS_cc_txn(where=(merch_name ne " ") drop= I022_POS_ENTRY);
...
data adhoc5.EOSS_dc_txn(where=(merch_name1 ne "") drop=ecom_indicator);
...
/*proc sql;*/
/*create table online_merchant_master as*/
/*select distinct merch_id,term_id,MERCH_NAME,merch_name*/
/*from adhoc5.shop_dc_online;*/
/*quit;*/
/**/
/*proc sql;*/
/*create table cc_online_mapping as*/
/*select distinct  t1.*,t2.merch_id as merch_key,t2.term_id as term_key,t2.MERCH_NAME*/
/*from (select monotonic() as tserno,* from  shop_cc_txn1) t1 left join online_merchant_master t2*/
/*on t1.MERCHANT_NUMBER=t2.merch_id or t1.tid=t2.term_id*/
/*order by tserno,merch_key desc,term_key desc;*/
/*/*quit;*/*/
/**/
/*PROC SQL;*/
/*   CREATE TABLE adhoc5.shop_cc_online AS */
/*   SELECT t1.tserno, */
/*          t1.ACCOUNT_NUMBER, */
/*          t1.CARD_NUMBER, */
/*          t1.BILLAMT, */
/*          t1.TRXN_DT, */
/*          t1.merch_name,*/
/*		  case when max(coalesce(merch_key,term_key)) ne " " then 1 else 0 end as online_txn*/
/*      FROM WORK.CC_ONLINE_MAPPING t1*/
/*group by 1,2,3,4,5,6;*/
/*QUIT;*/;

/*proc sql;*/
/*select case when BILLAMT<0 then "<0" else ">0" end as bucket,count(*)*/
/*from adhoc5.shop_cc_online*/
/*group by 1;*/
/*quit;*/;
